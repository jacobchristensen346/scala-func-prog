...
class ListsSuite extends munit.FunSuite:
  ...
  test("max of list with repeated elements") {
      assertEquals(max(List(4, 4, 4, 4)), 4)
    }
  ...
  test("max of a list with zeros and positives") {
      assertEquals(max(List(0, 5, 3, 0)), 5)
    }

  test("max of a list with positives and negatives") {
      assertEquals(max(List(-1, 2, -3, 4, -5)), 4)
    }
