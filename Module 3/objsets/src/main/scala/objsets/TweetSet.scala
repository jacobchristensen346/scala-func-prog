...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet =
    throw new java.util.NoSuchElementException("Empty set has no most retweeted tweet")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    val leftAcc = {
      if p(elem) then left.filterAcc(p, acc.incl(elem))
      else left.filterAcc(p, acc)
    }
    right.filterAcc(p, leftAcc)

  def union(that: TweetSet): TweetSet =
    /*val leftSide = left.union(that.incl(elem))
    right.union(leftSide)*/
  ...
  def mostRetweeted: Tweet =
    val bestLeft = {
      if (left.isInstanceOf[Empty]) then elem
      else {
        val leftMax = left.mostRetweeted
        if leftMax.retweets > elem.retweets then leftMax
        else elem
      }
    }
    if (right.isInstanceOf[Empty]) then bestLeft
    else {
      val rightMax = right.mostRetweeted
      if rightMax.retweets > bestLeft.retweets then rightMax
      else bestLeft
    }
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet =
    TweetReader.allTweets.filter((tw: Tweet) => google.exists((le: String) => tw.text.contains(le)))
  lazy val appleTweets: TweetSet =
    TweetReader.allTweets.filter((tw: Tweet) => apple.exists((le: String) => tw.text.contains(le)))
