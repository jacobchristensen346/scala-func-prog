...
class HuffmanSuite extends munit.FunSuite:
  ...
  test("frequencies of character list") {
    val freqList = List(('a', 2), ('c', 3), ('b', 1))
    val charList = List('a', 'c', 'a', 'c', 'b', 'c')
    assertEquals(times(charList), freqList)
  }
  ...
  test("create optimized CodeTree from list") {
    val charList = List('a', 'b', 'a', 'b', 'b')
    new TestTrees:
      assertEquals(createCodeTree(charList), t1)
  }

  test("decode a bit list") {
    val bitList = List(0, 0, 1, 1, 0)
    new TestTrees:
      assertEquals(decode(t1, bitList), List('a', 'a', 'b', 'b', 'a'))
  }

  test("decode a French secret") {
